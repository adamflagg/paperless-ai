...
router.post('/settings', express.json(), async (req, res) => {
  try {
    ...
    const restrictToExistingTags =
      req.body.restrictToExistingTags === 'on' || req.body.restrictToExistingTags === 'yes';
    ...
    const externalApiEnabled =
      req.body.externalApiEnabled === 'on' || req.body.externalApiEnabled === 'yes';
    ...
    if (
      normalizedPaperlessUrl !== currentConfig.PAPERLESS_API_URL?.replace('/api', '') ||
    ...
    if (systemPrompt !== undefined)
      updatedConfig.SYSTEM_PROMPT = processedPrompt.replace(/\r\n/g, '\n').replace(/\n/g, '\\n');
    ...
    updatedConfig.RESTRICT_TO_EXISTING_CORRESPONDENTS = restrictToExistingCorrespondents
      ? 'yes'
      : 'no';
    updatedConfig.RESTRICT_TO_EXISTING_DOCUMENT_TYPES = restrictToExistingDocumentTypes
      ? 'yes'
      : 'no';

...
class PlaygroundAnalyzer {
  ...
  async analyzeDocument(docCard, prompt) {
    ...
    try {
      ...
      const existingTags = Array.from(docCard.querySelectorAll('.tag')).map((tag) => ({
        id: tag.dataset.tagId,
        name: tag.textContent.trim(),
      }));

      const existingCorrespondent =
        docCard.querySelector('[data-correspondent]')?.dataset.correspondent;
    ...
    if (
      ...
      if (correspondentElem) {
        ...
        newValue.textContent =
          window.correspondentNames?.[analysisResult.correspondent] || analysisResult.correspondent;
